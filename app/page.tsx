...
export default function NursingApp() {
  ...
  // Use a ref to store ALL answers - this updates immediately!
  const allAnswersRef = useRef<Record<number, string[]>>({});
  ...
  const handleSelect = (optionId: string) => {
    ...
    // IMMEDIATELY save to ref (this is synchronous!)
    allAnswersRef.current[currentIndex] = newAnswers;
  };
  ...
  const handleSubmitAnswer = () => {
    ...
    allAnswersRef.current[currentIndex] = selectedAnswers;
    setIsCorrect(isAnswerCorrect);
    setShowRationale(true);
  };

  const handleNavigate = (index: number) => {
    // Save current answer to ref before navigating
    if (selectedAnswers.length > 0) {
      allAnswersRef.current[currentIndex] = selectedAnswers;
    }
    
    setCurrentIndex(index);
    const savedAnswers = allAnswersRef.current[index] || [];
  ...
  // FIXED: Submit mock test - use the ref which has ALL answers
  const handleSubmitMockTest = () => {
    // Save current question's answer to ref
    if (selectedAnswers.length > 0) {
      allAnswersRef.current[currentIndex] = selectedAnswers;
    }
    
    // Now the ref has ALL answers, submit
    setMockTestSubmitted(true);
    setCurrentView("results");
  };

  // FIXED: Calculate results using the ref
  const calculateResults = () => {
    // Use allAnswersRef.current for mock test
    const answersToUse = mockTestSubmitted ? allAnswersRef.current : {};
          ...
          <div style={{ marginTop: "1.5rem", paddingTop: "1rem", borderTop: "1px solid #e5e7eb" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
              {mockTestQuestions.map((_, idx) => {
                const isAttemptedQ = allAnswersRef.current[idx] && allAnswersRef.current[idx].length > 0;
